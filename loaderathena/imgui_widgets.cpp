...
namespace font
{
    ...
    extern ImFont* roleway_desc;
    extern ImFont* inter_medium;
}

bool ImGui::CustomButton(const char* label, const char* icon, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, IsItemActive() ? c::accent : c::accent, g.IO.DeltaTime * 6.f);

    GetWindowDrawList()->AddRectFilledMultiColor(bb.Min, bb.Max, GetColorU32(c::accent), GetColorU32(c::accent_inclined), GetColorU32(c::accent_inclined), GetColorU32(c::accent), c::widget::rounding);

    GetWindowDrawList()->AddText(ImVec2(bb.Min.x + ((size_arg.x - (CalcTextSize(label).x)) / 2 - 8), bb.Max.y - CalcTextSize(label).y - (size.y - CalcTextSize(label).y) / 2), GetColorU32(c::text::text_active), label);
    GetWindowDrawList()->AddText(font::icomoon, 18.f, ImVec2(bb.Min.x + ((size_arg.x + (CalcTextSize(label).x)) / 2 + 8), bb.Max.y - CalcTextSize(icon).y - (size.y - CalcTextSize(icon).y) / 2), GetColorU32(c::text::text_active), icon);
...
bool ImGui::Page(bool selected, const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    ...
    it_anim->second.text = ImLerp(it_anim->second.text, selected ? c::text::text_active : c::text::text, g.IO.DeltaTime * 6.f);
    it_anim->second.line = ImLerp(it_anim->second.line, selected ? 1.f : 0.f, g.IO.DeltaTime * 6.f);

    GetWindowDrawList()->AddRectFilled(bb.Min, bb.Max, GetColorU32(c::widget::background), c::widget::rounding);

    GetWindowDrawList()->AddRectFilled(bb.Min + ImVec2(size_arg.x / 3, size_arg.y - 3), bb.Max - ImVec2(size_arg.x / 3, 0), GetColorU32(c::accent, it_anim->second.line), c::widget::rounding, ImDrawFlags_RoundCornersTop);

    PushFont(font::roleway_desc);
    GetWindowDrawList()->AddText(ImVec2(bb.Min.x + (size_arg.x - (CalcTextSize(label).x)) / 2, bb.Max.y - CalcTextSize(label).y - (size.y - CalcTextSize(label).y) / 2 - 1), GetColorU32(it_anim->second.text), label);
    PopFont();
...
bool ImGui::icon_box(ImTextureID id_image, ImVec2 size, ImVec2 icon_size, ImU32 color_icon)
{
    ImGuiWindow* window = GetCurrentWindow();
    ...
    const ImGuiID id = window->GetID(id_image);
    ...
    ImGui::GetWindowDrawList()->AddRectFilled(rect.Min, rect.Max, GetColorU32(c::widget::background), c::widget::rounding);

    ImGui::GetWindowDrawList()->AddRectFilled(rect.Min + ImVec2(size.x / 3, size.y - 3), rect.Max - ImVec2(size.x / 3, 0), GetColorU32(c::accent, it_anim->second.line), c::widget::rounding, ImDrawFlags_RoundCornersTop);

    ImGui::GetWindowDrawList()->AddImage(id_image, rect.Min + ImVec2(icon_size / 2), rect.Max - ImVec2(icon_size / 2), ImVec2(0, 0), ImVec2(1, 1), GetColorU32(c::accent));
...
bool ImGui::Selector(bool selected, ImTextureID image, const char* label, const ImVec2& size_arg)
{
    ImGuiWindow* window = GetCurrentWindow();
    ...
    static std::map<ImGuiID, selector_state> anim;
    auto it_anim = anim.find(id);

    if (it_anim == anim.end())
    {
        anim.insert({ id, selector_state() });
        it_anim = anim.find(id);
    }

    it_anim->second.text = ImLerp(it_anim->second.text, selected ? c::text::text_active : c::text::text, g.IO.DeltaTime * 6.f);
    it_anim->second.line = ImLerp(it_anim->second.line, selected ? 0.2f : 0.f, g.IO.DeltaTime * 6.f);

    GetWindowDrawList()->AddRectFilled(bb.Min, bb.Max, GetColorU32(c::widget::background), c::widget::rounding);

    GetWindowDrawList()->AddRectFilledMultiColor(bb.Min, bb.Max, GetColorU32(c::accent, it_anim->second.line), GetColorU32(c::accent, 0.f), GetColorU32(c::accent, 0.f), GetColorU32(c::accent, it_anim->second.line), c::widget::rounding);

    GetWindowDrawList()->AddRect(bb.Min, bb.Max, GetColorU32(c::accent, it_anim->second.line * 8), c::widget::rounding);

    GetWindowDrawList()->AddImageRounded(image, bb.Min + ImVec2(10, 10), bb.Max - ImVec2(size_arg.x - 45, 10), ImVec2(0, 0), ImVec2(1, 1), GetColorU32(c::text::text_active), c::widget::rounding);

    GetWindowDrawList()->AddCircleFilled(ImVec2(bb.Max.x - (size_arg.y / 2), bb.Max.y - (size_arg.y / 2)), 12.f, GetColorU32(c::accent, it_anim->second.line), 100.f);
    GetWindowDrawList()->AddCircleFilled(ImVec2(bb.Max.x - (size_arg.y / 2), bb.Max.y - (size_arg.y / 2)), 8.f, GetColorU32(c::accent, it_anim->second.line * 2), 100.f);
    GetWindowDrawList()->AddCircleFilled(ImVec2(bb.Max.x - (size_arg.y / 2), bb.Max.y - (size_arg.y / 2)), 4.f, GetColorU32(c::accent, it_anim->second.line * 8), 100.f);


    PushFont(font::roleway_desc);
    GetWindowDrawList()->AddText(ImVec2(bb.Min.x + 60, bb.Max.y - CalcTextSize(label).y - (size.y - CalcTextSize(label).y) / 2 - 2), GetColorU32(it_anim->second.text), label);
    PopFont();

    return pressed;
}

void ImGui::status_list(const char* name, const char* description, const ImVec2& size, ImDrawFlags flags)
{
    ...
    GetWindowDrawList()->AddRectFilled(bb.Min, bb.Max, GetColorU32(c::widget::background), c::widget::rounding, flags);

    PushStyleColor(ImGuiCol_Text, GetColorU32(c::text::text_active));

    PushFont(font::inter_medium);

    RenderTextClipped(bb.Min + ImVec2(12, 0), bb.Max - ImVec2(0, 2), name, NULL, NULL, ImVec2(0.0, 0.5), &bb);

    RenderTextClipped(bb.Min, bb.Max - ImVec2(12, 2), description, NULL, NULL, ImVec2(1.0, 0.5), &bb);
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    if (axis == ImGuiAxis_X)
        ...
        grab_rect = ImRect(bb.Min.x - 5, ImLerp(bb.Min.y + 5, bb.Max.y, grab_v_norm), bb.Max.x - 5, ImLerp(bb.Min.y, bb.Max.y - 5, grab_v_norm) + grab_h_pixels);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (!is_multiline)
    {
        ...
        GetWindowDrawList()->AddRectFilled(frame_bb.Min + ImVec2(size_arg.x / 2.7f, size_arg.y - 3), frame_bb.Max - ImVec2(size_arg.x / 2.7f, 0), GetColorU32(c::accent, it_anim->second.line), c::widget::rounding, ImDrawFlags_RoundCornersTop);
